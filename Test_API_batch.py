...
def select_folder():
    root = tk.Tk()
    root.withdraw()
    return filedialog.askdirectory(title="選擇圖片資料夾")
...
def main():
    ...
    def process_folder(folder_path, folder_label):
        ...
        for img_file in image_files:
            ...
            if not extracted_items or any(
                "error" in item or not item.get("po_num") or not item.get("batch_num") or not item.get("expirydate")
